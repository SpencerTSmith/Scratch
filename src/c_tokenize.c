...
static C_Token_Type c_token_table[][3] =
{
  ...
  ['-'] = {C_TOKEN_SUBTRACT,           C_TOKEN_DECREMENT,     C_TOKEN_SUBTRACT_ASSIGN},
...
static
b32 c_lexer_char_valid_for_identifier(u8 c)
{
  // TODO: Make sure this is what is really valid
  return char_is_alphabetic(c) || char_is_digit(c) || c == '_';
}
  ...
  while (c_lexer_eat_whitespace(&lexer), c_lexer_incomplete(lexer))
  {
    ...
    else
    {
      if (char_is_alphabetic(curr_char) || curr_char == '_') // Identifier or keyword
      {
        ...
        for EACH_INDEX(keyword_idx, STATIC_COUNT(c_keywords))
        {
          C_Keyword_Info keyword = c_keywords[keyword_idx];

          usize end = MIN(lexer.at + keyword.string.count, lexer.source.count);
          String potential = string_substring(lexer.source, lexer.at, end);

          if (string_match(keyword.string, potential))
          {
        ...
        if (token.type == C_TOKEN_NONE) // Not a keyword, must be an identifier
        {
